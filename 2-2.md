...
- switch to vectored interrupts
- change trap frame handling without pre-allocated trap frames

### Realizations for the near future

- current style of pre-emption is bad 
- have a global var w pre-empt thing decrementing, xinu-style
- this is for sleep, yield reasons; re-programmign the quantum hardware would be funky
- hwever we should have a discussion abt the possiblity of just re-programming the quantum hardware
...
- VM_boot turns on FPU and vector unit... do we have this ???
...
- format of testbench
- how tohost works
- 2/14 interrupt realization -- hardware clears sstatus.sie...?
...
attribute__interrupd t an take an argument
- Riscv_explicit
For traps
- Up to us — whatever is easier for us to write
- Gnu.org

How do u get a compiler lol
- Riscv gcc —> he built it form the open source repository n stuff
- When u run module load riscvgcc it adds the stuff Cole built to path

c++ memory map array
- Class MemoryMap
- Used to store everything
- Map bcz memory is assumed to be sparsely uhh
- It’s a b-tree map???
- Bcz its ordered
- Whenever they do a read, they have read and write functions where they just take in address and either modify stuff in the map or just returning the value
- They also do checks n shit

Merilator: compiler verilog —> cpp code
1. Ventilator runs, generates all the cpp code and the makefile
2. Then u can add in ur own cpp code arbitrarily, but u need somethign that has a main function…
3. Then, the negerated makefile has ur specified list of cpp code + the stuff they generated…
...
```
set up supervisor trap entry
- we manually convert physical --> kernel virtual ddress 

Why do I have to manually convert the trap_entry addr, instead of passing the virtual &trap_entry? 

my question:
```
  write_csr(stvec, pa2kva(trap_entry));

  If -- after the MMU is enabled -- grabbing function pointers still returns the physical address that then needs to be converted to its virtual counterpart to be correct, doesn't that mean that all function pointers, variables, and everything else in memory has to be converted before using them? Or else the physical addrs will be interpreted as virutal addrs, which in turn map to the wrong physical addrs

I'm def reaching the wrong conclusion here because that defeats the point of the MMU, but I'm not sure where the logical jump is occurring.

```
to be answered.

Turns out m-mode doesn't have virtual addressing: from COLE: 

```
this is pretty confusing since we've got PAs and VAs going on, and they're using megapages.

Is this function running in M-mode by chance? If so it would bypass address translation entirely, which would explain why it can execute even without that. Otherwise I'd agree, the variable locations wouldn't be the same unless it was identity-mapped (which it doesn't seem to be)

"why would it bypass address translation for M-mode?"

that's just how M-mode works. the firmware doesn't want to see virtualized addresses, it wants real addresses. I believe there's a mode bit you can use to do page walks for assisting in debugging or doing services on behalf of the OS, but by default M-mode has physical access
...
can I be interrupted between vector entry and prologue in vectored interrupt mode, for which stack pushes/pops do not occur yet?
- no -- sstatus.sie is written to / triggered. cleared by hardware on trap entry. 
- how does this apply to nested interrupts? idk 
...
```
- writes the vector table as a trap entry
- traps are handled hre e in non-vector mode. (DIRECT MODE). 
- this is a little bit of a moot point, given that interrupts here are disabled with `write_csr(mie, 0`, so the test itself is mostly concerned with exceptions

`write_csr(sscratch, pa2kva(read_csr(mscratch))`
- sscratch is scratch CSR for software to manipulate at will
- its bcz during a privilege switch, we cant rlly trust the stack
- so we load sp (stack ptr) into sscratch
- this line converts whatever's in mscratch to virtual addressing, then pust it in sscratch
...
```
- delegates ecalls and all page faults to navigate to the supervisor-mode interrupt handler 
- in user mode, a fault will happen in s-mode
...
```
we need a pool of physical pages to hadn out when a page fault occurs? 

We already have these data structures:
```
...
```
- each node contains its PHYSICAL PAGE ADDRESS
  - this is because each ending page table entry contains the physical page number and permission bits. it doens't store virtual addrs because that's the entire point of translating them
  - the freelist manages physical memory invoentory -- which physical frames are used and free? 
- user_mapping tracks which physical page is mapped to each user virutal page
  - i is virtual page #i, and user mapping[i] is the physical page backign the virutal page
  - when the OS implements page eviction -- memory is full and a new page fault happens -- it chooses a victim virtual page, finds which physicla frame backs it, wirtes to disk, removes the mapping, and returns the physical frame to the free list
  - the MMU usses page tables to map virthal addrs --> physical, but it canot tell u which virtual page maps to a physical page and track the eviction policy.
  - the MMU is the one which raises a page fualt if virtual --> phsycial does not work, and this is what responds to it to switch the page out. 
  - and after it switches the page out, it updates the page table
    - the PTE for the victim virtual address is found, its valid bit (PTE_V) is cleared, flush TLB entry. 

Random seed is used to:

### what's with the trap frame 

1. in entry.S, trap frames are delegared per hart, and their addresses are loaded into the scratch registers 
2. when the trap entry occurs, the stack pointer is loaded from sscratch int o"sp"
3. stores the current context of the trap there

A trap frame is:
- snapshot of CPU state at the moment a trap occurred 
- stores all general-purples registesr and stuff

with bectored interrupts:
- trap entry pushes registers n shit, rather than having a pre-allocated trap frame

## 2/16

CLINT: core-local interrupt controller
- local interrupts per hart 
- usually machien timer interrupts (MTIP) and software interrupts (MSIP)
  - each hart gets its own mtimecmp register for timer interrupts
  - each hart gets its own msip bit for software interrupts 
- there is one CLINT block in the system, but it contains sep registers per hart

PIC: platform-level interrupt controller 
- external interrupts from devices: GPIO, UART, SPI...
- instead of wiring to every hart, devices go to PLIC, and the PLIC decides whic hart gets the interrupt
- PLIC is fully shared, but each hart has its own enable bits
